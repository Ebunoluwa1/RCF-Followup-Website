  ...
  return (
    <div className="max-w-7xl flex flex-col mx-auto px-4 sm:px-6 lg:px-8 py-11 bg-white shadow-md rounded-lg ">
      <div className="flex flex-row items-center justify-between">
        ...
        <main>
          {" "}
          {currentQuestion + 1} of {tests.length}
        </main>

        <button
          onClick={handleStart}
          className="flex justify-between items-center  mb-1 bg-[#D895E9] rounded-full shadow-md transition duration-300 py-1 px-2"
        >
      ...
      <div className="relative w-full h-2 bg-gray-300 rounded-full my-6">
        <div
          className="absolute top-0 left-0 h-full bg-purple-700 rounded-full transition-all duration-500"
      ...
      <div className=" flex flex-col justify-center">
        <div>
          <TextComponent
            className="my-2"
            text={tests[currentQuestion].questions}
          />
          ...
          <ul className="flex flex-col gap-4">
            {tests[currentQuestion].answers.map((answer, idx) => (
              <li
                key={idx}
                className="rounded-md border p-3 hover:cursor-pointer transition hover:bg-gray-100"
              >
                {" "}
                {answer.title}
              </li>
      ...
      <div className="flex flex-row items-center justify-center gap-4 my-8">
        <button
          ...
          className={`w-96 bg-white border-[#8D2DA5]
         text-[#8D2DA5] border-2  hover:transition hover:duration-300  px-4 py-2  inline-block rounded-md font-semibold text-center ${
           currentQuestion === 0
             ? "opacity-50 cursor-not-allowed"
             : "hover:bg-[#8D2DA5] hover:text-white"
         } `}
        ...
        <Button
          onClick={handleNext}
          size="md"
          className={`w-96 bg-[#8D2DA5] text-white font-light ${
            currentQuestion === tests.length - 1
              ? "opacity-50 cursor-not-allowed"
              : "hover:bg-purple-700"
          }`}
          disabled={currentQuestion === tests.length - 1}
        >
          Next
        </Button>
      </div>
